	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('yijianfankui','新增')" type="primary" @click="add('/index/yijianfankuiAdd')">
				...
				<div class="list3 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						...
						<div class="item-info">
							...
							<div class="desc ql-snow ql-editor" v-html="item.yijianfankui"></div>
						</div>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`yijianfankui/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/yijianfankuiDetail', query: params});
			},
